...
**RFC822 wire:** после правок parse/serialize/IMAP FETCH в `threlium/mail/` — `scripts/check_mail_wire.sh` (статический gate: wire только через `threlium.mail`, без re-export из `mime_reform`).
