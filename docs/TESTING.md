...
**Что не реализовано** (план, не код): матрица ветвей FSM (`cli_exec`/HITL/subagent IRT-depth), тесты оркестрации и гонок (включая взаимодействие dispatch-скрипта (notmuch query) ↔ LightRAG-воркер `cur/`-only, [`INDEX.md` §5b](INDEX.md#5b-lightrag-worker)), сценарные эмбеддинги LightRAG, negative-пути (5xx/таймаут/обрыв сети), e2e сценария **сбоя handler'а** с проверкой **failed `threlium-work@*`** и записи в **journald** ([`INDEX.md` §5.6](INDEX.md#56-universal-error-handling-в-runnersworkerpy)), санкционированные узкие теги плейбука кроме **deploy**/**refresh**, Telegram/Matrix каналы, тесты ограничений повторного прогона из [PLAYBOOK.md §2.1.4](PLAYBOOK.md#214-известные-ограничения--то-что-действительно-не-покрывается-ни-a-ни-b).
