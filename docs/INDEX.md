...
2. **Query** (`enrich`): seed-план — LLM `enrich_task_plan` **до** графа; строка запроса — Jinja `lightrag/lightrag_query.j2` (user intent → seed subtasks → thread context), cap `trim_from_end_tokens` / `lightrag_query_budget`; один `aquery` с `lightrag/rag_response.j2` как `system_prompt` (**мягко** приоритизирует `thread:{tid}` и `From:` memory-mailbox'ов, не отключая cross-thread ассоциации shared-графа). Отдельного plan-LLM `enrich_query_plan` нет.
...
| **Enrichment backpack (`build_context_backpack_multipart`)** | Гранулярные MIME-части: `<user-message>`, `<graph-answer>`, leaf-`<{hash}@history>`, `<thread-memory>`, `<global-memory>`, `<response-state>`, `<task-init>`/`<task-state>`. `extract_part_by_content_id` извлекает отдельную часть по Content-ID; reasoning собирает промпт через char-cap per-field + Jinja2-шаблон (см. [§7.3](#73-composing-the-enrichment-payload-granular-multipart); [`FSM.md` §5.2](FSM.md#52-контракт-тела-enrich--reasoning)). |
