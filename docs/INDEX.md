...
| Письмо после исключения в handler ([§5.6](#56-universal-error-handling-в-runnersworkerpy)) | остаётся в `new/` с `+unread` (`nm_settle` не вызывается); отдельного error-mail нет | сбой виден в journald; воркер с `Restart=on-failure`/`RestartSec=1s` **без** sweep; dispatch подберёт повтор после успеха или следующей доставки |
...
3. **`threlium.runners.engine_submit`** выходит ненулём → без `OnSuccess`/sweep; при **`Restart=on-failure`** systemd повторит submit с паузой **`RestartSec=1s`**. Отдельных error-mail и стадии `errors/` нет.
...
**Контракт строки для `ainsert`** — RFC822 из `render_lightrag_ingest_document(msg, thread_term=…)`: заголовки и `Content-Type` собираются в Python из `EmailMessage` и сериализуются `email.policy.default` (тот же контракт, что у `BytesParser(policy=default)` в чанкере); **текст тела** — вывод `lightrag/ingest_body.j2` (см. [ADR 0001](adr/0001-lightrag-ingest-chunking-enrich.md), [§7.6](#76-per-thread-scoping-soft-через-маркеры)). В **синтетической** копии для индексации добавляется заголовок `X-Threlium-Thread-Id` (на Maildir/notmuch **не** пишется). Чанкинг (`threlium_email_chunking_func`) парсит ту же строку, режет **plain**-тело по токенам tokenizer'а LightRAG и к каждому фрагменту приклеивает префикс с `Thread-Id` / `From` / `To` / `Subject` / `X-Threlium-LightRAG-Chunk` (номер чанка 1..N внутри документа) / …; при несовпадении с контрактом — `ValueError` (fail-fast). Размер окна тела и overlap задаются `THRELIUM_LIGHTRAG_CHUNK_BODY_*` и согласуются с `LightRAG(chunk_token_size=…, chunk_overlap_token_size=…)`.
...
2. **`<graph-answer>`** — JSON envelope из `_build_lightrag_envelope` (включает `threlium.formulated_query` и ответ `rag.aquery`).
...
- Передача в LiteLLM: `litellm.completion(..., tools=tools, tool_choice="auto")`. Успешное завершение стадии — только валидный `tool_call` по одному из маршрутов; plain-text без tool — `ReasoningStageError` (tool-only).
...
| **Synthetic ingest document** | RFC822-текст для `ainsert`: копия заголовков в `EmailMessage` + `X-Threlium-Thread-Id` + тело из `render_prompt("lightrag/ingest_body.j2")`, сериализация `policy.default` (согласована с чанкером). Заголовок `X-Threlium-Thread-Id` только в этой строке (не в Maildir). См. [ADR 0001](adr/0001-lightrag-ingest-chunking-enrich.md), [§5b.3](#5b3-цикл-индексации). |
