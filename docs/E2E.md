...
так тест проверяет инвариант по **state** (через Admin `GET /contexts/{name}`), а не сканируя журнал.
...
- **Счётчик попаданий** без арифметики: на каждый матч `list: { addLast: { hit: "1" } }` → число =
  `listSize` (special-property; читается helper-ом `state property='listSize'` или Admin `GET /contexts`).
