...
**Ingress distill tool bridge:** `tool_choice=required` + `ingress_distill_tool_spec.j2`; … на письме ingress→enrich (bridge-only): metadata + **`user_intent`** в `<history>` + **`<user-query>` CID** (ingress создаёт из bridge `<system>` через `enrich_user_query_from_bridge_system`, не relay с bridge); …
...
**Enrich-input VO (`fsm_strings.py`):** … :class:`~threlium.types.fsm_strings.EnrichUserQueryText` — канонический сырой user turn в fixed CID `<user-query>` на каждом `To: enrich@` (emit: `require_value`; read: `require_enrich_user_query_text` в `mime_reform`); **не** путать с :class:`~threlium.types.reasoning.ReasoningUserMessageText` (отрендеренный `<user-message>` в Jinja enrich→reasoning). :class:`~threlium.types.fsm_strings.EnrichCalleeHistoryText` — собственный `<history>` callee на X→enrich (error notice). :class:`~threlium.types.fsm_strings.EnrichRequestEchoText` — request_echo (subagent_intent). Единый emit: `emit_to_enrich` / `emit_enrich_validation_error` в `fsm_emit_semantic.py` (typed VO, без голого `str` в публичном API).
