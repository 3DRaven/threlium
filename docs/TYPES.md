...
**Capabilities:** строка заголовка `X-Threlium-Capabilities` — тип `ThreliumCapabilitiesBudgetLine`. Хвост (последний пробельный токен) = текущий уровень capability (аналогично `HopBudgetLine`). `subagent_intent` при создании фрейма добавляет токен для нового уровня; `subagent_end` при возврате копирует hop/cap родителя 1-в-1. **Лимиты capability (`settings.cap.root` / `settings.cap.sub`):** в `fsm_emit.py` читаются из `ThreliumSettings` и сразу используются при сборке строки заголовка hop/cap — намеренная граница: значения не протаскиваются через весь стек `advance_hop_budget_*` / `push_subagent_*`, чтобы не раздувать сигнатуры.
