...
**LiteLLM score ladder (multi-endpoint hosts):** `score` 0 → `targets.cli_hitl_resume` (HITL reply classifier, малые `max_tokens`); `score` 1 → `enrich_plan`, `enrich_task_hypotheses`, `lightrag_llm`, `summarize_context`, `response_observe`; `score` 2 → `reasoning`. E2E correlation: `LitellmCallSite.CONFIRM_CLI_HITL` = `confirm_cli_hitl` (WireMock-стабы). Стадия `enrich` делает **два** tool-вызова score 1 c одной формой args, но разными `function.name` / routing / промптами: `enrich_task_plan` (seed до RAG) и `enrich_task_hypotheses` (late verification-гипотезы после RAG); оба пишут в один `<task-init>` через `TaskInitOp` (отдельные `EnrichTaskPlanToolArgs` / `EnrichTaskHypothesesToolArgs`, см. § enrich tool bridge ниже).

**Unified required-tool вызов:** все исходящие chat completion с tools идут через `threlium.litellm_required_tool` (`build_site_call` / `invoke_required_tool` / `ainvoke_required_tool`): `tool_choice=required`, **ровно один** tool spec на HTTP-вызов, `correlation_with_call_site` выставляет `X-Threlium-Call-Site` = `tools[0].function.name`. Инвариант e2e (`_assert_single_tool_call_site` в `merge_litellm_call_kwargs_and_log`): для single-tool вызова header **строго** равен `function.name`; единственное исключение — reasoning multi-tool (`call_site = LitellmCallSite.REASONING`). Значения `LitellmCallSite` для tool-вызовов = `function.name`; `fsm` / `lightrag_index` / `lightrag_query` / `lightrag_query_rerank` — pipeline-маркеры только для не-tool вызовов (embedding / rerank / TLS-fallback). Каждый tool — автономный `prompts/**/tools/*_tool_spec.j2` (полный JSON, без `{% include %}`).

**Ingress distill tool bridge:** `tool_choice=required` + `ingress_distill_tool_spec.j2`; парсинг — `ingress_distill_tool_bridge.parse_ingress_distill_assistant` → `IngressDistillToolArgs` → `IngressDistillResult` (`types/ingress_distill.py`). Порядок history-частей на письме ingress→enrich (bridge-only): **`original_user_message`** (`distill_history_original_user.j2`) → metadata (`user_reply_language`, `step_back_notes`, `open_gaps`) → **`USER_INTENT`** в `<history>` (`IngressDistillHistoryPartKind`). **User query не в distill tool** — fixed CID **`<user-query>`** attach'ится отдельно: ingress создаёт `EnrichUserQueryText` из bridge `<system>` через `ingress_bridge_user_query.enrich_user_query_from_bridge_system` + `emit_bridge_distill_to_enrich` (преобразованный VO, не IRT-relay). Fallback при ошибке bridge — `ingress_distill_fallback_history_parts`.

**Bridge → ingress:** payload — **только `<system>`** (`build_bridge_ingress_email`, `email._build_canonical`); `<user-query>` на bridge-input **запрещён** (`assert_bridge_input_has_no_user_query`).

**Enrich-input VO (`fsm_strings.py`):** три VO на границе callee → `enrich@` — :class:`~threlium.types.fsm_strings.EnrichUserQueryText` (наследует `_RequiredNonEmpty`): **семантика текущего enrich-turn** (local turn callee) в fixed CID `<user-query>` на **каждом** `To: enrich@` — bridge: преобразованный `<system>`; internal: reflect body, validation notice, subagent task/result и т.д.; emit — `EnrichUserQueryText.require` / `from_external_body`; read — `require_enrich_user_query_text` в `mime_reform`; **не** путать с :class:`~threlium.types.reasoning.ReasoningUserMessageText` (отрендеренный `<user-message>` в Jinja enrich→reasoning). :class:`~threlium.types.fsm_strings.EnrichCalleeHistoryText` — опциональный собственный `<history>` callee (request-echo, relay history, …). :class:`~threlium.types.fsm_strings.EnrichRequestEchoText` — request_echo (`subagent_intent`). Choke-points: `emit_to_enrich`, `emit_bridge_distill_to_enrich`, `emit_enrich_validation_error` в `fsm_emit_semantic.py` (typed VO, без голого `str` в публичном API). Wire `SummarizeContextStagePayload.user_query` — plain `str`; decode boundary → `EnrichUserQueryText.require` в `_parse_payload`.

**Enrich tool bridge:** стадия `enrich` — `tool_choice=required` + автономные `prompts/lightrag/tools/*_tool_spec.j2`; `EnrichToolFunctionName` (`enrich_query_plan` / `enrich_task_plan` / `enrich_task_hypotheses`); парсинг — `enrich_tool_bridge.parse_enrich_*_assistant` (`require_single_tool_call` → `parse_tool_call` → `LiteLlmToolCallArgumentsWire.from_tool_call` → `validate_tool_args_json` → `msgspec.convert`). Отдельные VO `EnrichTaskPlanToolArgs` / `EnrichTaskHypothesesToolArgs` (одинаковая форма `subtasks: list[str]`, разный смысл/site/промпт — DDD). Late-проход `enrich_task_hypotheses` (после RAG) пишет в **тот же** `<task-init>` через `TaskInitOp`, не `tasks_upsert`; тексты → `TaskSubtaskText` / `TaskSubtaskContentId.from_text`, дедуп против seed+существующих; fail-open `[]` на ошибке bridge.
...
**LightRAG tool bridge:** фазы LLM — `tool_choice=required` + Jinja `prompts/lightrag/tools/*_tool_spec.j2`; внутри — `LightragToolFunctionName` (`StrEnum`), две **отдельные** VO для KG-извлечения — `ExtractKnowledgeGraphEntityToolArgs` (first pass, tool `extract_knowledge_graph`) и `ExtractKnowledgeGraphGleaningToolArgs` (gleaning, tool `extract_knowledge_graph_gleaning`) — каждая со своим автономным `*_tool_spec.j2`, плюс `SummarizeDescriptionsToolArgs` / `ExtractQueryKeywordsToolArgs` / `GenerateRagAnswerToolArgs` (`types/lightrag_tool_args.py`); фаза → spec/Args через `lightrag_tool_phase` (`detect_lightrag_call_site_wire` по `keyword_extraction` / `has_history` / `has_system_prompt`); wire VO (`LightragExtractionDelimiterText`, …) в `types/lightrag_tool_wire.py`; единственный `str` для библиотеки — `.value` на выходе `build_llm_func`. Ответ LiteLLM: `litellm_tool_response.require_tool_calls_response` (ровно один `tool_call`, `finish_reason=tool_calls`); при bridge-ошибках — до 2 повторов completion в `build_llm_func`. Сырой JSON args: :class:`~threlium.types.litellm_tool_call.LiteLlmToolCallArgumentsWire`. `max_tokens` — только `LlmEndpoint` слота `lightrag_llm`.
