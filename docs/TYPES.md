...
**LiteLLM score ladder (multi-endpoint hosts):** `score` 0 → `targets.cli_hitl_resume` (HITL reply classifier, малые `max_tokens`); `score` 1 → `enrich_plan`, `lightrag_llm`, `summarize_context`, `response_observe`; `score` 2 → `reasoning`. E2E correlation: `LitellmCallSite.CONFIRM_CLI_HITL` = `confirm_cli_hitl` (WireMock-стабы).
