...
```json
{
  "request": {
    ...
    "bodyPatterns": [
      { "contains": "Reply with ONLY the question text to send to the graph." }
    ],
    "customMatcher": {
      "name": "state-matcher",
      "parameters": {
...
| `/chat/completions` ingress distill (`075`) | `X-Threlium-Call-Site: ingress_distill` + `"tools"` + `ingress_distill` in body; ответ `finish_reason: tool_calls` |
