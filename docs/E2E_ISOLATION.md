...
```json
{
  "request": {
    ...
    "headers": {
      "X-Threlium-Call-Site": { "equalTo": "fsm" }
    },
    "bodyPatterns": [
      { "contains": "Reply with ONLY the question text to send to the graph (no preamble" }
    ],
    "customMatcher": {
      "name": "state-matcher",
      "parameters": {
...
| `/chat/completions` `085` entity | `hasContext` + `X-Threlium-Call-Site: lightrag_index_entity` |
| `/chat/completions` `097` glean  | `hasContext` + `X-Threlium-Call-Site: lightrag_index_gleaning` |
| `/chat/completions` `095` summarize | `hasContext` + `X-Threlium-Call-Site: lightrag_index_summarize` |
| `/chat/completions` `090` keywords | `hasContext` + `X-Threlium-Call-Site: lightrag_query_keywords` |
| `/chat/completions` `082` rag response | `hasContext` + `X-Threlium-Call-Site: lightrag_query_response` |
...
| `/chat/completions` ingress distill (`075`) | composite `hasContext` + `X-Threlium-Call-Site: fsm` + `"tools"` + `ingress_distill` in body; ответ `finish_reason: tool_calls` |
...
┌─ pytest setup ─────────────────────────────────────────────┐
│                                                            │
│  1. upsert_wiremock_mapping_directory(stub_dir)            │
│  2. wiremock_state_seed_context(stub_tag::route) → active  │
│  3. [matrix/tg] register_room / register_update → list     │
│                                                            │
├─ test body ────────────────────────────────────────────────┤
│                                                            │
...
│                                                            │
├─ pytest_sessionfinish (глобально, один раз) ───────────────┤
│                                                            │
│  8. Дождаться idle threlium-work@ / user workers +         │
│     пустой ``GET /__admin/requests/unmatched`` (опрос).    │
│  9. DELETE /__admin/state-extension/contexts (все).         │
└────────────────────────────────────────────────────────────┘
```
...
### 8.1. WireMock `matches` — full-match семантика
...
### 8.2. Handlebars: литеральная `{` перед `{{#block}}`
...
### 8.3. Bootstrap стабы и selective reset
...
### 8.4. test_id в теле сообщения для coverage assertion
...
### 8.5. Notmuch-дедупликация при повторяющемся /sync
...
### 8.6. Запрет поля `priority` в JSON-стабах сценариев
...
`X-Threlium-Call-Site`, URL и узкие `bodyPatterns` (chunk / enrich-plan tail).
...
### 8.7. Эмуляция «долгого LLM»: цепочка **307 Temporary Redirect** (polling без длинного держания сокета)
