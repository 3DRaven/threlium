...
Исходный код расширения — `vendor/wiremock/wiremock-state-extension` (gitignore); в compose
монтируется JAR `wiremock-state-extension-standalone-0.10.1.jar` (ServiceLoader, без `--extensions`).
Справочник по API и возможностям расширения — §9.
...
**Enrich seed plan** (`081_chat_enrich_task_plan.json`, сценарий `hitl_matrix`; `080`/`enrich_query_plan` удалён — теперь один Jinja `lightrag_query.j2` без plan-LLM):
...
```json
{
  "request": {
    ...
    "headers": {
      "X-Threlium-Call-Site": { "equalTo": "enrich_task_plan" }
    },
    "bodyPatterns": [
      { "contains": "\"tools\"" }
    ],
...
Стадия `enrich` делает в одном hop два tool-вызова с одинаковой формой args, но разными `X-Threlium-Call-Site`: `enrich_task_plan` (seed, до RAG; стаб `081_*`) и `enrich_task_hypotheses` (late verification-гипотезы, после RAG; стаб `083_*`). Для каждого нужен **свой** стаб с тем же `hasContext`, разделённый по `X-Threlium-Call-Site`. Routing site `enrich_plan` (`LitellmRoutingSite.ENRICH_PLAN`) — для seed LLM; call-site / `function.name` = `enrich_task_plan`. Hypotheses — отдельный call-site `enrich_task_hypotheses` (`target_score: 1.0`). RAG warmup в mailflow: при `rag_bytes < 32768` после первого `aquery` — `wait_for_rag_warmup` (`tests/e2e/toolkit/mailflow.py`, `_RAG_WARMUP_SKIP_MIN_BYTES`).
...
Критично: «фаза 1» и «фаза 2» — **разные JSON-стабы**, а не один стаб с двумя
`priority`. WireMock при совпадении нескольких mapping'ов без явного `priority`
берёт **последний добавленный**; здесь в каждый момент времени property `phase_X`
либо отсутствует, либо есть — поэтому `hasNotProperty: phase_X` и
`hasProperty: phase_X` **не пересекаются** и одновременно не матчат (§9.6.1).
...
| `/chat/completions` enrich seed plan (`081`) | composite `hasContext` + `X-Threlium-Call-Site: enrich_task_plan` + `"tools"`; ответ `finish_reason: tool_calls` (`080`/`enrich_query_plan` удалён) |
| `/chat/completions` enrich hypotheses (`083`) | composite `hasContext` + `X-Threlium-Call-Site: enrich_task_hypotheses` + `"tools"`; ответ `finish_reason: tool_calls` |
...
**Вместо этого:** **взаимоисключающие** фазовые стабы на одном контексте
`{stub_tag}::{{request.headers.[x-threlium-thread-root]}}`: один стаб с
`hasNotProperty: phase_X`, другой — с `hasProperty: phase_X` (§5), плюс дискриминаторы
`X-Threlium-Call-Site`, URL и узкие `bodyPatterns`. Это **не** подмена механизма
`priority`: если два mapping'а матчат один запрос одновременно, WireMock (default
priority = 5) отдаёт **последний зарегистрированный** стаб; модель держится на том,
что state делает фазы **disjoint**. Подробнее — §9.6.1.
...
## 9. WireMock State Extension: модель и инструментарий

Раздел — справочник по [wiremock-state-extension](https://github.com/wiremock/wiremock-state-extension)
(исходники в `vendor/wiremock/wiremock-state-extension`, upstream README — там же). Threlium использует
расширение вместо WireMock Scenarios: нужно хранить **данные** между стабами (properties, shared list) и
матчить запросы по состоянию, а не только по `requiredState` сценария.

### 9.1. Модель данных

| Термин | Описание |
| ------ | -------- |
| `context` | Имя «ключа» в Store (строка). В Threlium — составной `{stub_tag}::{correlation_key}` или shared (`matrix_rooms`, `telegram_updates`). |
| `state` | Одна карта `property → value` (строки) на контекст. Повторный `recordState` **мерджит** свойства. |
| `property` | Поле внутри `state`. Значение `"null"` (строка) **удаляет** свойство. |
| `list` | Упорядоченный список карт `{property: value}` в том же контексте. Элементы **только** добавляются (`addFirst`/`addLast`) или удаляются — **in-place правки** элемента нет. |
| `updateCount` | Счётчик изменений контекста (+1 за запрос, если был хотя бы один write в этом контексте). |

```
Store
 └── context: "stub-A::route_X"
       ├── state: { active: "1", phase_nested: "1" }
       └── list: [ {room_id: "!e2e_…"}, … ]
```

Хранилище — `CaffeineStore` (in-memory, lock на весь Store). Standalone: TTL по умолчанию 60 мин
(`WIREMOCK_STATE_EXTENSION_CONTEXT_EXPIRATION_SEC` — экспериментально). Не рассчитано на распределённый
WireMock; для параллельных потоков — **разные контексты** (§1, §9.6).

### 9.2. Компоненты (`StateExtension`)

Один `ExtensionFactory` регистрирует шесть расширений:

| Имя | Тип | Назначение |
| --- | --- | ---------- |
| `recordState` | `ServeEventListener` | Запись/обновление `state` и/или `list` **до** отправки ответа клиенту. |
| `deleteState` | `ServeEventListener` | Удаление контекста или элемента `list`. |
| `stateTransaction` | `ServeEventListener` (global) | Очистка transaction-метаданных после завершения запроса. |
| `state-matcher` | `RequestMatcherExtension` | `customMatcher` — матчинг по наличию контекста и его содержимому. |
| `state` | Handlebars helper | Чтение property/list в `response-template` и во **всех** параметрах расширения. |
| `stateAdminApi` | `AdminApiExtension` | Read/delete Store через Admin API (§9.5). |

Требуется `--global-response-templating` (в compose включён). Standalone JAR подхватывается через ServiceLoader.

### 9.3. `recordState` — запись состояния

Вызывается в `serveEventListeners` стаба. Шаблоны Handlebars в `context`, `state.*`, `list.*`;
модель: `request` (как в WireMock) + `response` (`body`, `headers`).

**Параметры:**

| Параметр | Действие |
| -------- | -------- |
| `context` | Обязателен. Имя контекста (после рендера шаблона). |
| `state` | `{ "prop": "value", … }` — merge properties. `"prop": "null"` удаляет `prop`. |
| `list.addLast` / `list.addFirst` | Добавить элемент (карту properties) в list контекста. |

`state` и `list` можно комбинировать в **одном** listener (один increment `updateCount`).
Несколько `recordState` в одном стабе допустимы, но **порядок listener'ов не гарантирован** —
следующий listener видит уже записанное предыдущим.

**Threlium:**

- Seed LiteLLM-контекста: bootstrap `000_e2e_state_setup.json` → `active: "1"`.
- Сброс одного property: `001_e2e_state_phase_reset.json` → `phase_tasks_ledger_done: "null"`.
- Фазовый автомат reasoning: `recordState` в `serveEventListeners` LLM-стабов (§5).
- Matrix/Telegram shared list: `addLast` в `matrix_rooms` / `telegram_updates` (§3, §4).
- POST-триггеры сценария (`/__threlium/e2e/state/reasoning_release` и т.п.) — тот же `recordState`.

### 9.4. `deleteState` — удаление

Тоже `serveEventListeners`. Два режима (взаимоисключающих на верхнем уровне):

**Удаление контекста целиком** (нет ключа `list`):

| Параметр | Действие |
| -------- | -------- |
| `context` | Один контекст. |
| `contexts` | Массив имён (неизвестные — игнор). |
| `contextsMatching` | Regex по именам (Java `Pattern`). |

**Удаление из `list`** (есть `list`, обязателен `context`):

| Ключ в `list` | Действие |
| ------------- | -------- |
| `deleteFirst: true` | Первый элемент. |
| `deleteLast: true` | Последний элемент. |
| `deleteIndex: "N"` | Индекс (`0`, `-1` = последний). Невалидный индекс — тихий no-op. |
| `deleteWhere: { property, value }` | **Первый** элемент, где `element[property] == value`. |

**Threlium:** teardown Matrix/Telegram — `deleteWhere` по `room_id` / `update_id` (§3.6, §4.4).
Полный flush Store — только `DELETE /__admin/state-extension/contexts` в `pytest_sessionfinish`
(`wiremock_state_reset_all_contexts`), не из тела теста.

### 9.5. Admin API

Базовый путь: `/__admin/state-extension/` (без PUT/POST — запись только через стабы).

| Endpoint | Метод | Ответ |
| -------- | ----- | ----- |
| `/contexts` | GET | JSON-массив имён контекстов. |
| `/contexts/{context}` | GET | Структура контекста (`state` + `list`) или `404`. |
| `/contexts` | DELETE | Удалить **все** контексты, `204`. |
| `/contexts/{context}` | DELETE | Удалить один контекст, `204`. |

**Gotcha Threlium:** `DELETE …/contexts/{name}` с именем, содержащим `::`, `<`, `>`, `@`
(типичный составной ключ и MID), часто **no-op** — WireMock отдаёт `204`, но контекст остаётся
(имя в path не доходит до handler'а). Для точечного снятия property — POST-триггеры
(`phase_reset`, `recordState` с `"null"`), не Admin DELETE. См. `composite_context_key` в
`tests/e2e/wiremock_client.py`.

### 9.6. `state-matcher` — матчинг запросов

`customMatcher` с `"name": "state-matcher"`. Имя контекста в `hasContext` / `hasNotContext`
рендерится Handlebars **до** проверки (можно `{{request.headers.[x-threlium-thread-root]}}`).

**Базовые предикаты** (на контексте из `hasContext`):

| Параметр | Смысл |
| -------- | ----- |
| `hasContext` | Контекст **существует** (достаточно одного имени). |
| `hasNotContext` | Контекста **нет**. |
| `hasProperty` / `hasNotProperty` | Ключ есть/нет в `state`. |
| `property` | `{ "key": { "equalTo": "…", "contains": "…", … } }` — любой WireMock `StringValuePattern`. |
| `list` | `{ "first"\|"last"\|"-1"\|"0": { "key": { …matcher… } } }` — property элемента list. |
| `updateCountEqualTo` / `LessThan` / `MoreThan` | Счётчик обновлений контекста. |
| `listSizeEqualTo` / `LessThan` / `MoreThan` | Длина `list`. |

**Логика:** вложенные `and` (массив, все должны совпасть), `or` (хотя бы один), `not` (инверсия).
Внутри одного flat-объекта с `hasContext` все **разные** дополнительные ключи
(`hasProperty`, `hasNotProperty`, `property`, …) агрегируются через AND
(`MatchResult.aggregate` в `StateRequestMatcher`).

#### 9.6.1. Gotcha: `hasProperty`/`hasNotProperty` ≠ замена `priority`

Три разных уровня — их легко смешать:

**1. Один flat-объект `parameters` (один `hasContext`).** JSON допускает **не более одного**
ключа каждого имени: второй `"hasNotProperty"` в том же объекте **перезапишет** первый
(останется только последнее значение). Пара **разных** ключей `hasProperty` + `hasNotProperty`
в одном объекте **работает** — оба предиката проверяются (AND). Пример:
`100_chat_reasoning_cli_intent.json` — `hasProperty: active` и
`hasNotProperty: phase_cli_intent_done` вместе.

**2. Несколько проверок одного типа** (два `hasNotProperty`, два `hasProperty`) — только через
`"and": [ { "hasContext": "…", "hasNotProperty": "a" }, { "hasContext": "…", "hasNotProperty": "b" } ]`
или через `property` с несколькими ключами. Пример:
`101_chat_reasoning_post_cli_observe.json`.

**3. Несколько mapping JSON (уровень WireMock).** `state-matcher` решает, **матчит ли данный
стаб** запрос. Если **два разных стаба** матчат **один и тот же** запрос (одинаковые
`urlPathPattern`, headers, body и оба проходят `state-matcher`), WireMock **не** смотрит на
state дальше: при default `priority` (= 5 у всех) побеждает **последний зарегистрированный**
mapping (`upsert_mapping` = remove+add; каталог грузится по имени файла — `102_` позже `100_`).

Фазовый автомат §5 **не использует priority**, потому что стабы **взаимоисключающие по state**:
пока `phase_X` нет — матчит только стаб с `hasNotProperty: phase_X`; после `recordState`
— только стаб с `hasProperty: phase_X`. Одновременный double-match — ошибка проектирования
(нужны дополнительные дискриминаторы или `and`, а не `priority`).

**Threlium:** основной дискриминатор — `hasContext: "{stub_tag}::{{request.headers.[x-threlium-thread-root]}}"`;
фазы — disjoint-пары стабов (§5). `listSizeMoreThan` **не** используем на bootstrap
`/sync` и `getUpdates` (§3.2, §4.2) — пустой list должен отвечать, а не unmatched.

### 9.7. Handlebars helper `state` — чтение в ответах

Регистрируется как helper `state` (в шаблонах: `{{state …}}` / `{{{state …}}}` для сырого JSON).

| Параметр | Обязателен | Описание |
| -------- | ---------- | -------- |
| `context` | да | Имя контекста. |
| `property` | XOR `list` | Обычное property или спец.: `updateCount`, `listSize`, `list` (весь массив). |
| `list` | XOR `property` | JSONPath по list, напр. `'[0].room_id'`, `'[-1].text'`. |
| `default` | нет | Fallback при отсутствии контекста/property. |

**Спец-properties:**

| property | default без контекста | default при пустом list |
| -------- | --------------------- | ------------------------ |
| `updateCount` | `"0"` | — |
| `listSize` | `"0"` | `0` |
| `list` | `[]` | `[]` (или `default='[]'`) |

**List → JSON-ответ:** только в поле `body` (строка), **не** в `jsonBody`. Паттерн Threlium —
`response-template` + `{{#each (state context='matrix_rooms' property='list' default='[]')}}` (§3.2).
Для валидного JSON между элементами — `{{#unless @last}},{{/unless}}`. Литеральная `{` перед
`{{#each}}` требует пробела (§8.3).

В matcher'ах и `recordState`/`deleteState` helper `state` тоже доступен (можно читать предыдущее
состояние **до** записи в том же listener — порядок: шаблон → write).

### 9.8. Сопоставление возможностей расширения и паттернов Threlium

| Возможность расширения | Паттерн в e2e |
| ---------------------- | ------------- |
| `hasContext` + Handlebars | Изоляция LiteLLM/Matrix egress по `{stub_tag}::thread-root` (§2.2, §3.5) |
| `hasProperty` / `hasNotProperty` | Фазовый автомат внутри одного треда (§5) |
| `recordState` + `"null"` | `phase_reset`, снятие одной защёлки без удаления контекста (§2.1) |
| `list` + `{{#each (state … property='list')}}` | Единый bootstrap `/sync`, `getUpdates` (§3, §4) |
| `list.addLast` / `deleteWhere` | `register_room` / `unregister_room`, telegram register/unregister |
| `property` / `list` matchers | Доступны, в Threlium предпочитаем `hasProperty` + `X-Threlium-Call-Site` |
| `updateCount*` / `listSize*` | Доступны; в Threlium не используем для изоляции (§6) |
| `and` / `or` / `not` | Несколько `hasNotProperty` в одном стабе; disjoint фазовые пары **между** стабами (§9.6.1), не замена WireMock `priority` |
| Admin GET `/contexts` | Отладка после падения e2e (`docker exec`, journal) |
| Admin DELETE all contexts | Cold reset / `pytest_sessionfinish` only |

### 9.9. Ограничения и отладка

- **Конкурентность:** lock на Store; атомарность — на уровне одного инстанса и одного контекста.
  Параллельные pytest-xdist workers на **одном** WireMock — разные `context` (§1); list-операции
  дополнительно сериализуются `_wiremock_admin_api_exclusive` (§3.2).
- **История state:** расширение не хранит версий — только текущий snapshot.
- **Admin write:** намеренно отсутствует; seed только через стабы (`/__threlium/e2e/…` или LLM stub).
- **Логи:** `verbose=true` на WireMock; extension пишет info по context (create/update/delete/list ops).
  Ошибки matcher/listener — WireMock notifier + `ConfigurationException`.
- **Upstream примеры:** `vendor/.../src/test/java/.../examples/`, JSON в `src/test/resources/`.
