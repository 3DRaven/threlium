...
Стадия `enrich` делает в одном hop два tool-вызова с одинаковой формой args, но разными `X-Threlium-Call-Site`: `enrich_task_plan` (seed, до RAG) и `enrich_task_hypotheses` (late verification-гипотезы, после RAG). Для каждого нужен **свой** стаб с тем же `hasContext`, разделённый по `X-Threlium-Call-Site` (`enrich_task_plan` / `enrich_task_hypotheses`); оба отвечают `finish_reason: tool_calls` с массивом `subtasks`. Маршрутизация LiteLLM `enrich_task_hypotheses` — `target_score: 1.0` (как `enrich_plan`).
