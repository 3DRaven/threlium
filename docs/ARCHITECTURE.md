...
| [SUBAGENT_TABLE.md](SUBAGENT_TABLE.md) | Матрица FSM делегирования, таблицы правил `ingress_router` / `egress_router`, маркеры `subagent_intent` / `subagent_end`, `X-Threlium-Hop-Budget` / `X-Threlium-Capabilities`, HITL.                                                           |
...
- **Саморазвитие агента.** Threlium имеет право модифицировать собственный код и конфигурацию в `threlium_repo_path` через стадию `cli_exec` — при условии, что capability-профиль текущего фрейма (§6, `X-Threlium-Capabilities`) такие операции разрешает, а ресурсные лимиты transient scope (`systemd-run --scope` с `MemoryMax`/`CPUQuota`/`TasksMax`, см. [ORCHESTRATION.md §5](ORCHESTRATION.md#5-гонки-восстановление-лимит-параллелизма)) допускают их исполнение. Применение правок — штатными `systemctl --user daemon-reload` (+ `restart` при необходимости) из той же цепочки `cli_exec` и локальный `git commit`. Запрещено профилем — правки делает только оператор, локально на target.
...
Мосты могут добавлять вторую MIME-часть — `text/plain` attachment с фиксированным именем файла (`RawIngressCaptureAttachmentFilename`): для почты — все заголовки входящего сообщения плюс только текстовое тело; для TG/Matrix — JSON маршрута и тела. Эта полная структура остаётся в durable-записи `stages/ingress/Maildir`; стадия `ingress` перед переходом в `enrich` / `cli_resume` нормализует исходящее письмо в одно `text/plain` (`ingress_pipeline_email`), так что дальше по FSM вложения не передаются.
...
- **L1, L2, … (субагенты).** Вызов субагента — маркер `subagent_intent` в IRT-цепочке с изолированным hop/cap. Субагент не знает, кто его вызвал. По завершении `reasoning` субагента отправляет ответ в `egress_router`, который по depth > 0 маршрутизирует в `subagent_end`; тот находит соответствующий `subagent_intent` по IRT, копирует hop/cap родителя 1-в-1 и возвращает письмо в `ingress@localhost`.
...
Всё формирование пользовательского/LLM-видимого контента писем (тела, динамические Subject, FSM/wire-format маркеры внутри тел) вынесено в Jinja2-шаблоны под раскладкой по стадиям FSM `$THRELIUM_HOME/prompts/<stage>/<purpose>.j2` (например, `reflect/continue.j2`, `reflect/final.j2`, `ingress/orphan_notice.j2`, `cli_intent/*.j2`, `cli_resume/*.j2`, `cli_hitl_out/*.j2`, `cli_exec/observation.j2`, `logic_validate/observation.j2`, `memory_query/observation.j2`, `subagent_intent/budget_exhausted.j2`, `reasoning/system.j2`, `reasoning/user.j2`, `reasoning/<route>/{tool_spec,email_body,email_subject}.j2`, `lightrag/ingest_body.j2`, `lightrag/enrich_*.j2`, `lightrag/mail_context.j2`). Конверт bridge→`ingress@` собирается в Python (`threlium.bridges.build_bridge_ingress_email`), не через Jinja.
...
- **LightRAG enrich** — цепочка `lightrag/enrich_query_plan.j2` → один LLM → `lightrag/enrich_aquery_user.j2` → `aquery` с `lightrag/rag_response.j2` как `system_prompt` → JSON envelope в `<graph-answer>` + `lightrag/mail_context.j2` для контекстных частей (без второго LLM в enrich; см. [`states/enrich.py`](../ansible/roles/threlium/files/scripts/threlium/states/enrich.py), [`INDEX.md` §7.5](INDEX.md#75-query-call-always-on)). Подсказки к user-вопросу графа — `THRELIUM_LIGHTRAG_AQUERY_HINTS` в `enrich_aquery_user.j2`.
...
- **Reasoning per-route tool-spec** — `prompts/reasoning/<route>/{tool_spec,email_body,email_subject}.j2` (6 routes × 3 файла = 18 артефактов). На каждый ключ из `ROUTE_TO_ADDRESS` отдельный JSON-tool-spec со своей JSON-Schema (`additionalProperties: false`, `maxLength`-лимиты), и пара `email_subject` + `email_body` для рендера итогового письма. Инвариант: `function.name == route_key` (identity mapping), нарушение → `RuntimeError` на старте. `litellm.completion(... tools=tools, tool_choice="auto")`; успешный путь — ровно один валидный `tool_call`. Ответ без `tool_call` (включая непустой plain-text) → `ReasoningStageError` (политика tool-only). Аргументы `tool_call` валидируются `jsonschema`; ошибки схемы пробрасываются как `jsonschema.ValidationError`.
- **Разделение WHEN / HOW для knowledge-маршрутов** (`memory_query`, `logic_validate`, `reflect`). Решение «какой маршрут выбрать» (decision-tree, MUST/MUST NOT, связь с hop budget) живёт в `prompts/reasoning/system.j2` — секции `<knowledge_route_decision>`, `<reflect_strategy>`, `<logic_validate_strategy>` и `<route_priorities>`. Описание «как заполнять аргументы» (SHACL/Turtle/SPARQL для `logic_validate`) держится компактным в `tool_spec.j2`, а развёрнутый авторинг-референс (`turtle_syntax.md`, `shacl_sparql.md`, `sparql_functions.md`) вынесен в индексируемую базу знаний и достаётся через `memory_query`. Так строка маршрута в списке инструментов остаётся короткой и не конкурирует за внимание модели с триггерами выбора.
...
| `cli_intent@localhost` | **Только политика**: `allow` → команда в `cli_exec`; `deny` → возврат в `reasoning` через `ingress`; «нужен человек» → HITL через `cli_hitl_out` → `egress_router`. **Команды не исполняет.** |
| `cli_exec@localhost`   | **Только исполнение** разрешённой команды; `observation` возвращается в `ingress@localhost` для последующего обогащения контекста.                                                            |


Полная матрица шагов CLI (включая HITL-прерывание, возврат ответа пользователя в `cli_resume` и управление hop/cap через `X-Threlium-Hop-Budget` / `X-Threlium-Capabilities`) — шаги 10–18 и правила роутеров в [SUBAGENT_TABLE.md](SUBAGENT_TABLE.md).

**Политика `cli_intent` (грубый фильтр).** Функция классификации `classify_cli_policy` намеренно использует крайне грубый алгоритм `allow`/`deny`: все аргументы `argv` склеиваются в одну строку и ищутся на точное вхождение запрещённых подстрок (переменная окружения `THRELIUM_CLI_DENY_PATTERNS`, по умолчанию `;`, `|`, `$(`, `&&` и т.д.). Разрешённые базовые команды задаются через `THRELIUM_CLI_ALLOWLIST`. Этот подход осознанно порождает ложные срабатывания (например, `echo ";"` будет заблокировано как потенциально опасное). Цель фильтра — максимальная жёсткость и предсказуемость в ущерб гибкости. Если команда не попала в `allow` или `deny`, она уходит на HITL-подтверждение к человеку.
