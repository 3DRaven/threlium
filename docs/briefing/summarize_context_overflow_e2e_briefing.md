...
[`user_query_semantic_turn_briefing.md`](user_query_semantic_turn_briefing.md) (ingress: `## Original user message` в unified),
[`e2e_toolkit_refactor_briefing.md`](e2e_toolkit_refactor_briefing.md) (harness: `toolkit/mailflow.py`, `toolkit/fixtures.py`).

**Дата работы:** 2026-06-02 (ре-калибровка e2e — 2026-06-03 после semantic user-query)  
**Smoke e2e:** `test_summarize_context_e2e.py::test_summarize_overflow_full_pipeline` — **PASSED** (~126 с), лог `test-runs/.summarize_overflow_investigate.log`
...
**2 prior-хода + 1 основной** (`summarize_overflow_prior_turns=2` в `SUMMARIZE_CONTEXT_SPEC`).

| Ход | Сырое тело inject | Distill-бриф (WireMock) | Эффект |
|-----|-------------------|-------------------------|--------|
| seed[0..1] | Малое (HEAD + PAD 500 + tail) | templated `user_query` ~1347 chars (все history) | Накопление в unified, **без** overflow |
| main | Плотное (`e2e_dense_threlium_ctx_body`) | тот же стаб, уникальный MID в brief | 3-е письмо → overflow → summarize |
...
- Оценка на письмо (tier1, **все** `<history>`, включая `## Original user message` после semantic-uq):
  distill ~1347 + raw bridge original (prior: HEAD + PAD 500 + tail) → **~1900+** `body_chars` + header 100
- **2 prior + main (3 письма в unified):** prior не триггерят summarize; overflow на **main** enrich
- **3 prior + main (4 письма):** при semantic-uq overflow срабатывает на **последнем prior** — см. § «Проблема 3»

В стабе **50** повторов `accumulation-filler` (было 60 — давало overflow уже на 3-м ходе).

### Проблема 3 — ре-калибровка после semantic user-query (2026-06-03)

**Симптом:** `test_summarize_overflow_full_pipeline` падал на assert «PAD не в post-summarize reasoning»,
хотя pipeline и `tag:context_summarized` проходили.

**Корневая причина:** ingress (`emit_bridge_distill_to_enrich`) кладёт **`## Original user message`**
(сырой bridge body с `E2E-SUM-ORIG-PAD-MARKER`) в `<history>`. `history_body_chars` считает это
честно → вес prior-хода выше, чем в калибровке «только distill ~1447». При `prior_turns=3` overflow
срабатывал на **seed 2**, а не на main.

**Почему PAD в reasoning:** цикл summarize **не меняет** `user_query` (CONTEXT_CONTRACT §5).
`user_query` в payload overflow = `<user-query>` того enrich-листа, где сработал overflow.
На prior-ходе там сырой bridge body **с PAD** → после re-trigger enrich PAD виден в
`<user_message>` reasoning, даже когда unified уже свёрнут.

**Фикс (только e2e):** `summarize_overflow_prior_turns: 3 → 2` — overflow снова на **main**
(dense body без PAD). Prod-фильтрация `## Original user message` из summarize input **не**
применялась (осознанное дублирование bridge body в unified).

**Assert:** PAD проверяется только в **post-summarize** reasoning-хопах (где уже есть
`E2E-SUM-CONTEXT-MARKER`), не во всех prior-turn reasoning POST в журнале.
...
```
tests/e2e/
  test_summarize_context_e2e.py   # summarize_overflow_prior_turns=2; post-summarize PAD assert
...
**Ассерты** (`_assert_summarize_pipeline_artifacts`):
...
- в reasoning (post-summarize hops): `E2E-SUM-CONTEXT-MARKER`, pad не протёк в `<user_message>` / unified.
...
| Что | Результат |
|-----|-----------|
| `test_summarize_overflow_full_pipeline` | **PASSED** ~126 s (solo после ре-калибровки prior_turns=2) |
...
4. **PAD в post-summarize reasoning:** overflow на prior-ходе, не на main — уменьшить `prior_turns`
   или пересчитать вес с `## Original user message`; не фильтровать original из prod summarize input.
