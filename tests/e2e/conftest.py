...
:func:`~tests.e2e.toolkit.coord.e2e_compose_coord_paths`); остальные воркеры читают
...
(:func:`~tests.e2e.toolkit.compose_lifecycle.discover_live_e2e_project_name`). Подъём + bake образа —
...
даёт ``pytest.fail`` — см. ``docs/E2E.md`` §4.2.
...
``docs/E2E.md``): каждый тест/запуск генерирует уникальный ``correlation_key``
...
def _leader_rollback(
    ...
    Следующий прогон: preflight :func:`~tests.e2e.toolkit.compose_lifecycle.stop_compose_projects_for_e2e_compose_dir`
